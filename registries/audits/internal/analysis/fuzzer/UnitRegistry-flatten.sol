// The following code is from flattening this file: UnitRegistry.sol
...
/// @author Aleksandr Kuperman - <aleksandr.kuperman@valory.xyz>
contract UnitRegistryFuzzing is GenericRegistry {
    ...
    function _getSubComponents(uint32 componentId) internal view returns (uint32[] memory subComponentIds) {
    ...
    function _checkDependencies(uint32[] memory dependencies, uint256 maxComponentId) internal  {
        uint256 lastId;
            ...
            for (uint256 i = 0; i < numUnits; ++i) {
                ...
                for (; processedComponents[i] < numComponents[i]; ++processedComponents[i]) {
                    if (minComponent < components[i][processedComponents[i]]) {
                        ...
                        numComponentsCheck++;
                        if(maxNumComponents > type(uint32).max || numComponentsCheck > type(uint32).max || numUnits > type(uint32).max || minIdxComponent > type(uint32).max) {
